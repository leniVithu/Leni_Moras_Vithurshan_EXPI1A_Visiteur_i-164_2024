...
CREATE TABLE IF NOT EXISTS `t_visite` (
  ...
  `fk_Visiteur` int DEFAULT NULL,
  `ID_PersonneContact` int DEFAULT NULL,
  PRIMARY KEY (`ID_Visite`),
  KEY `ID_PersonneContact` (`ID_PersonneContact`),
  KEY `ID_Visiteur` (`fk_Visiteur`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Listage des données de la table lenimoras_vithurshan_expi1a_gestion_visiteur.t_visite : ~10 rows (environ)
INSERT INTO `t_visite` (`ID_Visite`, `Date_de_Visite`, `Heure_d_Arrivee_Prevue`, `Heure_d_Arrivee_Reelle`, `Duree`, `Heure_de_Depart`, `Motif_de_Visite`, `Frequence_de_visites`, `Nom_du_Batiment`, `fk_Visiteur`, `ID_PersonneContact`) VALUES
...
-- Listage des données de la table lenimoras_vithurshan_expi1a_gestion_visiteur.t_visiteur : ~0 rows (environ)
...
-- Listage des données de la table lenimoras_vithurshan_expi1a_gestion_visiteur.t_visiteur_societe : ~0 rows (environ)
